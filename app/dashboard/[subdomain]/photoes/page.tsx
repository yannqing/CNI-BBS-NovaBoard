...
import { CreatePostRequest, PublishPostStepOneRequest } from "@/types/post/post";
import { createPostAction, getTagsAction, publishPostStepOneAction } from "./action";
...
export default function DashBoardPhotosPage({
  ...
  const fetchCategorys = async () => {
    const response: BaseResponse<BasePage<Category>> = await getAllCategoryAction(getAllCategoryRequest);
    if (response.success) {
  ...
  const fetchTags = async () => {
    const response: BaseResponse<BasePage<GetTagsResponse>> = await getTagsAction(getTagsRequest);
    if (response.success) {
  ...
  const publishPostStepOne = async () => {
    const response: BaseResponse<string> = await publishPostStepOneAction(publishPostStepOneRequest);
    if (response.success) {
  ...
  const publishPost = async () => {
    const response: BaseResponse<string> = await createPostAction(publishPostRequest);
    ...
    <div>
      <main>
        ...
        <div className="m-4 gap-2">
          <div>标题</div>
          <div>
            <Input placeholder="请输入标题" onValueChange={(value) => {
              setPublishPostRequest(prev => ({
                ...prev,
                title: value,
              }));
        ...
        <div className="m-4 gap-2">
          <div>输入摘要</div>
          <div>
            <Input placeholder="请输入摘要" onValueChange={(value) => {
              setPublishPostRequest(prev => ({
                ...prev,
                summary: value,
              }))
        ...
        <div className="m-4 gap-2">
          ...
          <div>
            <Select 
              className="max-w-xs" 
              label="选择一个类别" 
              selectedKeys={publishPostRequest.categoryId ? new Set([publishPostRequest.categoryId]) : new Set()} 
        ...
        <MarkdownEditor getContent={(content) => {
          setPublishPostRequest(prev => ({
            ...prev,
            content: content,
          }));
